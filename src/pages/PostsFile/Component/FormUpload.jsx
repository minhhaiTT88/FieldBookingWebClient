...
import { Avatar, Button, Checkbox, Form, Image, Input, List, Select, Space, Upload } from "antd";
...
export const FormUpload = ({ form }) => {
  ...
  const handleBeforeUpload = (file, files) => {
    ...
    if (fileList.length === 500 || files.length >= 500) {
      toast.error("Vui lòng tải lên tối đa 500 file 1 lần.");
      return LIST_IGNORE;
    }
    // custom rename file name
    var newName = moment().format("YYYYMMDD_hhmmssa") + "_" + makeid(6);
    // replace origin File
